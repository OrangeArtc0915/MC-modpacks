...
execute if score temp0 int matches 1.. at @s anchored eyes positioned as @e[tag=tmp,limit=1] run summon area_effect_cloud ^-1.5 ^-1.5 ^-0.5 {Tags:["linese","instant","mpar_green_dust","mpar_custom","tmp1"],Duration:10,Motion:[0.4d,0.0d,0.0d]}
execute if score temp0 int matches ..0 at @s anchored eyes positioned as @e[tag=tmp,limit=1] run summon area_effect_cloud ^1.5 ^-1.5 ^-0.5 {Tags:["linese","instant","mpar_green_dust","mpar_custom","tmp1"],Duration:10,Motion:[0.4d,0.0d,0.0d]}
...
execute as @e[tag=tmp,limit=1] at @s anchored eyes run playsound minecraft:block.pumpkin.carve player @a ~ ~ ~ 1 2.0

scoreboard players set min int -20
scoreboard players set max int 120
