...
#减10并保证不为负
execute store result score temp int run data get entity @e[tag=tmp,limit=1] Item.tag.result.entity.sp
scoreboard players remove temp int 10
execute store result entity @e[tag=tmp,limit=1] Item.tag.result.entity.sp float 1 run scoreboard players operation temp int > 0 int
