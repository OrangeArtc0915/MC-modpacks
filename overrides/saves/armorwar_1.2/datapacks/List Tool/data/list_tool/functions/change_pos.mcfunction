...
execute if score @s list_input matches ..-1 run data modify entity @e[type=armor_stand,limit=1,tag=list_tool_marker] ArmorItems[0].tag.input prepend from entity @s Item.tag.input
#无论如何，把第一项放到最后
...
execute if score @s list_input matches ..-1 run function list_tool:change_pos_again
execute unless score @s list_input matches ..-1 run function list_tool:change_pos