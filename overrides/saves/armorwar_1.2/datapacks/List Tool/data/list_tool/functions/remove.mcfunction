#pre
summon armor_stand ~ ~ ~ {Tags:["list_tool_marker"],Marker:1b,Invisible:1b,NoGravity:1b,Invulnerable:1b,ArmorItems:[{id:"structure_void",Count:1b,tag:{input:[]}},{},{},{}]}
data modify entity @e[type=armor_stand,limit=1,tag=list_tool_marker] ArmorItems[0].tag.input set from entity @s Item.tag.input
...
#judge
execute if score @s list_input matches 0.. run function list_tool:remove_pos
execute if score @s list_input matches ..-1 run function list_tool:remove_neg
