...
execute as @s at @s run data modify entity @e[type=armor_stand,limit=1,tag=list_tool_marker] ArmorItems[0].tag.input set from entity @s Item.tag.result
scoreboard players operation list_input list_input = @s list_input
execute store result score @e[type=armor_stand,limit=1,tag=list_tool_marker] list_input if data entity @s Item.tag.result[]
...
#judge
execute if score @s list_input matches 0.. as @s at @s run function list_tool:change_pos
execute if score @s list_input matches ..-1 as @s at @s run function list_tool:change_neg
