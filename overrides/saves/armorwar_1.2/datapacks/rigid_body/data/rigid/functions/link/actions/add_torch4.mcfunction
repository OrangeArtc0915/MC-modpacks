...
execute as @e[tag=torch04] if score @s rigid_mo = @e[tag=tmp4,limit=1] rigid_id run tag @s add tmp5

execute unless entity @e[tag=tmp5] run summon armor_stand 0 11 0 {Marker:1b,Tags:["tmp","test5","torch0","torch04"],ArmorItems:[{},{},{},{id:"minecraft:torch",Count:1b}]}
